...
This section will focus on some simple but import plots, mostly involving counts
and frequencies of measures. This will give a good overview of our data at a 
high level before we delve into specifics and multivariate interactions.
...
##### Track Durations and Album Runtimes {.tabset}
...
df %>%
  ...
  summarise(
    ...
    count = n()) %>%
  pivot_longer(cols = c(avg_dur, count)) %>%
  ggplot(
    aes(
      x = stringr::str_wrap(album_name, 9) %>% reorder(album_release_date),
      y = value,
...
```{r album-trends, fig.height=8, results = 'hide', fig.show = 'all'}
album_trends <- df %>%
  group_by(album_name) %>%
  summarise(
    album_name,
...
avg_track_len_plot <- album_trends +
  ...
  scale_y_continuous(
    ...
    labels = c("2.0e+05", "2.4e+05", "2.8e+05", "3.2e+05")) +
  ggthemes::theme_clean()
...
```{r display-plots, results = 'hide', fig.keep = 'all'}
